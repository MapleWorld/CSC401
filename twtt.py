...
def twtt(output_file_name, input_file_name, SID):
...
if __name__ == "__main__":
    ...
    #twtt('testdata_output.csv', 'testdata.manual.2009.06.14.csv', 999768327)
    #twtt('training_output.csv', 'training.1600000.processed.noemoticon.csv', 999768327)
    twtt('./output/test_output.csv', 'test.csv', 999768327)
    

    '''
    input0 = 'Argh� Fantasy Surfer Carnage� Dane, Marlon &amp; Nat C all out!  #fantasysurfer converted HTML To ASCII: &'
    input1 = '@switchfoot http://twitpic.com/2y1zl - <t>asda</t> - @gg www.omg.com &apos; Awwwf Third Day to!!! +- do &gt; it. ;D #omg'
    input2 = "This tweet's the best tweet!! Everybody says so."
    
    new_line = twtt1(input0)
    new_line = twtt2(new_line)
    new_line = twtt3(new_line)
    new_line = twtt4(new_line)
    ...
    if len(sentences) > 0:
        sentences = [twtt7(sentence) for sentence in sentences]
        sentences = [twtt8(sentence) for sentence in sentences]
    ...
    '''
    sentence_1 = re.sub(r"((["+ string.punctuation + "])\\2*)", r" \1 ", input).strip()
    print sentence_1
