function myRunFn(dir_hmm, D)
    % Input
    % dir_hmm : The directory of where the trained data for phonemes are located
...
function writeToFile(dir_hmm, percent_correct)
    % Open file
    file_output = fopen('./runResult.txt', 'a+');
    
    % Write to file 
    fprintf(file_output, 'File Name: %s Percent: %f \n', dir_hmm, percent_correct);
    fclose(file_output);
end
